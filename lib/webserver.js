...
module.exports = (config, archiveManager) => {
  ...
  app.get('/', (req, resp) => {
    ...
    if (config.testDomain) domain = domain = config.testDomain
  ...
  app.get('/fullGeo', (req, resp) => {
    let domain = req.headers.host
    if (config.testDomain) domain = domain = config.testDomain
    archiveManager.findArchive(domain, (archiveError, archive) => {
      fullGeo(config, archiveError, domain, archive, req, resp)
    })
  })


  app.get('/html/live.js', (req, resp) => {
    let filename = path.resolve('./html/live.js')
...
function handleHtmlRoot (config, archiveError, domain, archive, req, resp) {
  ...
  Domain.mode(archive, domain, (error, info) => {
    ...
    let developer = {
      ...
      datVersion: archive.version,
      dat: `dat clone dat://${archive.key.toString('hex')}`,
      dns: `dig ${domain} TXT`,
      api: `curl -H "Accept: application/json" ${domain}`
    }
    ...
    if (info.isCollectionWithIndex) {
      return geojson.center(archive, info.isCollectionWithIndex, (err, geo) => {
        ...
        Collection.asDomainList(config, archive, info.isCollection, domain, (err, collection) => {
          ...
          return resp.send(Mustache.render(config.template, {config, developer, place: generatePlace(domain), geo, collection, links, properties, hasProperties, hasGeojson, isCollection}))
        })
      })
      ...
      return geojson.center(archive, info.isLeaf, (err, geo) => {
        ...
        let properties = geojson.allProperties(geo.geojson)
        geo.geojson = JSON.stringify(geo.geojson)
        developer.dat = developer.dat + info.isLeaf
        geo.box = JSON.stringify(geo.box)
        let hasProperties = properties.length
        let hasGeojson = true
        let isCollection = false
        return resp.send(Mustache.render(config.template, {config, developer, place: generatePlace(domain), geo, links, properties, hasProperties, hasGeojson, isCollection}))
      })
    } else if (info.isCollection) {
      return Collection.asDomainListWithGeoInfo(config, archive, info.isCollection, domain, (err, collection, geo) => {
        ...
        let properties = geojson.allProperties(geo.geojson)
        developer.dat = developer.dat + info.isCollection
        geo.geojson = JSON.stringify(geo.geojson)
        geo.box = JSON.stringify(geo.box)
        let hasProperties = false
        let hasGeojson = false
        let isCollection = true
        let data = {config, developer, place: generatePlace(domain), geo, collection, links, properties, hasProperties, hasGeojson, isCollection}
        let content = Mustache.render(config.content, data)
        data.content = content

        return resp.send(Mustache.render(config.template, data))
      })
    }
...
function fullGeo(config, archiveError, domain, archive, req, resp) {
  if (archiveError) return resp.status(404).send({error: archiveError})
  if (config.disableCollectionListing) return resp.status(403).send({ok: false, reason: 'collectionListings is disabled'})
  Domain.mode(archive, domain, (error, info) => {
    if (error) onErrorJson(error, resp)
    Collection.asDomainListWithGeoInfo(config, archive, info.isCollection, domain, (err, collection, geo) => {
      resp.send(geo)
    })
  })
}

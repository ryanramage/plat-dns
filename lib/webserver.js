    ...
    if (info.isCollectionWithIndex) {
      return geojson.center(archive, info.isCollectionWithIndex, (err, geo) => {
        ...
        Collection.asDomainList(config, archive, info.isCollection, domain, (err, collection) => {
          ...
          return resp.send(Mustache.render(config.template, {config, place: generatePlace(domain), geo, collection, links, properties, hasProperties, hasGeojson, isCollection}))
        })
      })
      ...
      return geojson.center(archive, info.isLeaf, (err, geo) => {
        ...
        return resp.send(Mustache.render(config.template, {config, place: generatePlace(domain), geo, links, properties, hasProperties, hasGeojson, isCollection}))
      })
    } else if (info.isCollection) {
